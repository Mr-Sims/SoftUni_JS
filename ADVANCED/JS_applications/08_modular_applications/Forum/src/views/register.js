import { register } from '../api/api.js';
import { inputField } from '../common/input.js';
import { html } from '../lib.js';
import { createSubmitHandler } from '../util.js';

const registerTemplate = (onSubmit, errorMsg, errors, values) => html`
<div class="narrow drop center">
    <header><h1>Register</h1></header>
    <form @submit=${onSubmit}>
        ${errorMsg ? html`<p class="error-msg">${errorMsg}</p>` : null}
        ${inputField('Email', 'email', 'email', values.email, errors.email)}
        ${inputField('Display Name', 'text', 'displayName', values.displayName, errors.displayName)}
        ${inputField('Password', 'password', 'password', values.password, errors.password)}
        ${inputField('Repeat', 'password', 'repass', values.repass, errors.repass)}
        <input class="action" type="submit" value="Sign up">
    </form>
</div>`;

export function showRegister(ctx) {

    update();

    function update(errorMsg, errors = {}, values = {}) {
        ctx.render(registerTemplate(createSubmitHandler(
            onSubmit,
            'email',
            'displayName',
            'password',
            'repass'
        ), errorMsg, errors, values));
    }

    async function onSubmit(data, event) {

        try {
            const missing = Object.entries(data).filter(([k, v]) => v == '');

            if (missing.length > 0) {
                const errors = missing.reduce((a, [k]) => Object.assign(a, { [k]: true }), {});
                throw {
                    error: new Error('All fields are required!'),
                    errors: errors
                }
            }
            if (data.password != data.repass) {
                throw {
                    error: new Error('Passwords do not match!'),
                    errors: {
                        password: true,
                        repass: true
                    }
                }
            }

            await register(data.email, data.displayName, data.password);0
            event.target.reset();
            ctx.updateUserNav();
            ctx.page.redirect('/topics');


        } catch (err) {
            const message = err.message || err.error.message;
            update(message, err.errors, data);
        }

    }

}